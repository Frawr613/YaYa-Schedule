(function (global) {
  ...
  const originalEffectAnchors = {
    ...
    cache: ["localStorage", "日期索引", "DDL 索引", "日程/特殊变更状态分桶", "DDL 视图状态", "已完成 DDL 检索状态", "HTML 局部缓存", "浮窗惰性 HTML 缓存", "空浮窗快速跳过", "自动定位签名缓存", "滚动标记节流", "强制渲染滚动后合并", "同帧渲染请求合并", "架构运行时签名去重", "模板装配签名缓存", "主题变量签名缓存", "前台恢复合并", "权限状态 UI 去重", "通知相同载荷跳过桥接", "滚动延迟渲染闸门", "主题变量缓存", "模板签名缓存", "原生桥接去重缓存", "Service Worker"],
  ...
  const layers = [
    ...
    },
    {
      ...
      name: "本地状态/缓存层",
      files: ["app.js", "sw.js"],
      owns: ["localStorage 状态", "日期索引", "DDL 索引", "日程/特殊变更状态分桶", "DDL 页签状态", "已完成 DDL 检索状态", "HTML 局部缓存", "浮窗惰性 HTML 缓存", "空浮窗快速跳过", "自动定位签名缓存", "当天条目缓存", "滚动标记节流", "强制渲染滚动后合并", "同帧渲染请求合并", "架构运行时签名去重", "模板装配签名缓存", "主题变量签名缓存", "前台恢复合并", "权限状态 UI 去重", "通知相同载荷跳过桥接", "滚动延迟渲染闸门", "主题/模板应用签名缓存", "原生桥接去重缓存", "Service Worker 优先缓存"],
      provides: ["当天日程读取", "日程/特殊变更进行中计数", "DDL 快照", "DDL 进行中/已完成快照", "已完成 DDL 检索快照", "局部渲染快照", "浮窗惰性 HTML 快照", "空浮窗无操作快照", "自动定位去重签名", "滚动标记低频刷新", "滚动后合并强制渲染", "渲染 RAF 合并", "架构运行时去重签名", "模板装配去重签名", "主题变量去重签名", "前台恢复合并触发", "权限状态 UI 去重签名", "通知桥接去重签名", "滚动后仅补积压渲染", "主题/模板去重签名", "原生桥接去重签名", "离线资源"],
