(function (global) {
  ...
  const originalEffectAnchors = {
    ...
    interaction: ["玻璃浮窗", "本地丝滑弹出动画", "独立自定义主题面", "底部/侧边抽屉", "居中面板", "日期时间选择器", "模板输入浮层", "内置选项弹层", "滚动锁定", "滚动锁定签名去重", "顶层滚动白名单", "选择器轻量定位", "选择器滚动合并", "锁定滚动恢复合并", "自动定位签名去重", "视图状态无变化跳过", "日期聚焦无变化跳过", "浮窗开关无变化跳过", "滚动标记节流", "强制渲染滚动后合并", "同帧渲染请求合并", "滚动不触发日程卡重绘", "渲染运行时节流", "模板 DOM 去重应用", "空浮窗快速跳过", "浮窗惰性 HTML 生成", "教务页内确认浮窗", "视窗锁定拖动", "教务网页不锁操作"],
