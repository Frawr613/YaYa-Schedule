(function (global) {
  ...
  const originalEffectAnchors = {
    ...
    cache: ["localStorage", "缓存结构版本", "日期索引", "按日期特殊变更索引", "DDL 索引", "完成项索引缓存", "备注计数缓存", "日程/特殊变更状态分桶", "DDL 视图状态", "已完成 DDL 检索状态", "已完成 DDL 筛选无变化跳过", "已完成 DDL 筛选结果缓存", "HTML 局部缓存", "浮窗惰性 HTML 缓存", "空浮窗快速跳过", "自动定位签名缓存", "概览分组局部签名缓存", "日程卡备注渲染隔离", "备注原生桥接跳过", "视图状态无变化跳过", "日期聚焦无变化跳过", "权限提示无变化跳过", "提醒载荷缓存", "通知队列去重", "滚动标记节流", "强制渲染滚动后合并", "同帧渲染请求合并", "架构运行时签名去重", "模板装配签名缓存", "主题变量签名缓存", "缓存重建签名跳过", "前台恢复合并", "权限状态 UI 去重", "通知相同载荷跳过桥接", "滚动延迟渲染闸门", "主题变量缓存", "模板签名缓存", "原生桥接去重缓存", "Service Worker"],
