(function (global) {
  ...
  const originalEffectAnchors = {
    source: ["教务导入", "文件导入", "确认学期", "教务页内确认学期", "结构化 DOM 采集", "iOS 教务资源载荷读取", "iOS 教务资源超时保护", "网页导出优先", "分页源数据兜底", "网页导出回传", "网页型 XLS", "伪 XLS HTML 解析", "MHTML 编码兜底", "文件导入松散解析"],
    domain: ["课程分组", "考试学期分类", "考试学科名", "常驻日程", "日程进行中/已结束", "DDL 进行中/已完成", "已完成 DDL 内容/时间检索", "特殊变更进行中/已结束", "移动/取消变更", "取消已设置变更", "普通备注"],
    ...
    components: ["顶部状态", "DDL 条", "DDL 分页面板", "已完成 DDL 检索面板", "今日面板", "课程/考试/日程/任务概览", "任务概览状态分页", "课程概览打开时本学期定位", "考试概览打开时本学期定位", "考试概览 chip 自动定位", "考试概览学期分页", "考试信息收口", "课程选课人数隐藏", "状态分组概览卡", "主题配色面板", "模板化内置输入/选项组件"],
    interaction: ["玻璃浮窗", "本地丝滑弹出动画", "独立自定义主题面", "底部/侧边抽屉", "居中面板", "日期时间选择器", "模板输入浮层", "内置选项弹层", "滚动锁定", "滚动锁定签名去重", "顶层滚动白名单", "选择器轻量定位", "选择器滚动合并", "锁定滚动恢复合并", "自动定位签名去重", "视图状态无变化跳过", "日期聚焦无变化跳过", "浮窗开关无变化跳过", "滚动标记节流", "强制渲染滚动后合并", "同帧渲染请求合并", "滚动不触发日程卡重绘", "渲染运行时节流", "模板 DOM 去重应用", "空浮窗快速跳过", "浮窗惰性 HTML 生成", "教务页内确认浮窗", "视窗锁定拖动", "教务网页不锁操作", "iOS 手机触控排版"],
    ...
    platform: ["原生 WebView", "通知", "后台/系统提醒管理", "提醒载荷缓存", "通知队列去重", "应用图标", "iOS Widget 刷新合批", "iOS 提醒重复挂载去重", "iOS 通知相同载荷跳过桥接", "备注原生桥接跳过", "iOS 前后台提醒轻量恢复", "iOS 前台恢复合并", "iOS 权限状态查询节流", "iOS 权限状态 UI 去重", "iOS 教务状态回写去重", "iOS 教务回传拉取合并", "iOS 教务桥接能力探测", "iOS 键盘避让桥接", "iOS WebKit 进程恢复", "iOS POST 新窗口保真", "iOS 文件导入取消静默", "iOS 文件导入格式收口", "iOS 网页导出暂存回传", "iOS App锁缩放/教务自由缩放", "教务单层网页登录", "教务主题桥接去重", "iOS 教务触控自动释放", "iOS 手机操作显示适配"],
  ...
  const layers = [
    ...
    },
    {
      ...
      owns: ["状态条", "DDL 条", "DDL 页签面板", "已完成 DDL 检索面板", "日期查询", "日期选择器", "时间选择器", "内置选项组件", "今日面板", "概览卡", "课程概览打开时定位", "考试概览学期分页", "状态分组概览卡", "任务概览状态分页", "设置浮窗", "主题配色面板", "备注卡片"],
      provides: ["可排序模块", "可隐藏模块", "功能面板 DOM", "DDL 分页 DOM", "已完成 DDL 检索 DOM", "课程分组卡片", "考试学期分页 DOM", "课程概览打开定位 DOM", "日程/特殊变更/任务状态页 DOM", "模板输入组件 DOM"],
