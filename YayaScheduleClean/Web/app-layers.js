(function (global) {
  ...
  const originalEffectAnchors = {
    ...
    cache: ["localStorage", "日期索引", "DDL 索引", "日程/特殊变更状态分桶", "DDL 视图状态", "已完成 DDL 检索状态", "HTML 局部缓存", "浮窗惰性 HTML 缓存", "空浮窗快速跳过", "自动定位签名缓存", "滚动延迟渲染闸门", "主题变量缓存", "模板签名缓存", "原生桥接去重缓存", "Service Worker"],
    ...
    interaction: ["玻璃浮窗", "本地丝滑弹出动画", "独立自定义主题面", "底部/侧边抽屉", "日期时间选择器", "模板输入浮层", "内置选项弹层", "滚动锁定", "滚动锁定签名去重", "顶层滚动白名单", "选择器轻量定位", "自动定位签名去重", "滚动不触发日程卡重绘", "渲染运行时节流", "模板 DOM 去重应用", "空浮窗快速跳过", "浮窗惰性 HTML 生成", "教务页内确认浮窗", "视窗锁定拖动"],
    ...
    platform: ["原生 WebView", "通知", "后台/系统提醒管理", "应用图标", "小组件", "Widget 去重刷新", "iOS Widget 刷新合批", "iOS 提醒重复挂载去重", "iOS 前后台提醒轻量恢复", "iOS 权限状态查询节流", "iOS 教务状态回写去重", "教务单层网页登录", "教务主题桥接去重"],
  ...
  const layers = [
    ...
    },
    {
      ...
      name: "本地状态/缓存层",
      files: ["app.js", "sw.js"],
      owns: ["localStorage 状态", "日期索引", "DDL 索引", "日程/特殊变更状态分桶", "DDL 页签状态", "已完成 DDL 检索状态", "HTML 局部缓存", "浮窗惰性 HTML 缓存", "空浮窗快速跳过", "自动定位签名缓存", "当天条目缓存", "滚动延迟渲染闸门", "主题/模板应用签名缓存", "原生桥接去重缓存", "Service Worker 优先缓存"],
      provides: ["当天日程读取", "日程/特殊变更进行中计数", "DDL 快照", "DDL 进行中/已完成快照", "已完成 DDL 检索快照", "局部渲染快照", "浮窗惰性 HTML 快照", "空浮窗无操作快照", "自动定位去重签名", "滚动后仅补积压渲染", "主题/模板去重签名", "原生桥接去重签名", "离线资源"],
    ...
    },
    {
      ...
      owns: ["浮窗层级", "独立自定义主题面", "底部抽屉", "侧边抽屉", "居中面板", "选择器浮层", "滚动隔离", "滚动锁定签名去重", "顶层滚动白名单", "选择器轻量定位", "自动定位签名去重", "滚动不触发日程卡重绘", "渲染运行时节流", "模板 DOM 去重应用", "空浮窗快速跳过", "浮窗惰性 HTML 生成", "滑动操作", "重渲染保护"],
      provides: ["modalLayout", "density", "浮窗置顶", "下层冻结", "滚动渲染闸门", "运行时刷新预算", "模板应用去重", "空浮窗无操作", "浮窗内容惰性生成", "自动定位去重", "选择器轻量定位", "轻触反馈", "繁忙降噪"],
    ...
    },
    {
      ...
      owns: ["原生保存", "通知", "后台/系统提醒管理", "应用图标", "小组件", "Widget 去重刷新", "iOS Widget 刷新合批", "iOS 提醒重复挂载去重", "iOS 前后台提醒轻量恢复", "iOS 权限状态查询节流", "iOS 教务状态回写去重", "教务 WebView", "单层网页登录", "教务主题桥接去重", "Web 预览降级"],
      provides: ["统一平台 API", "原生能力探测", "后台提醒状态", "后台提醒状态节流", "Widget 更新", "Widget 去重更新", "Widget 刷新合批", "教务导入回传", "教务状态回写去重", "教务主题桥接去重", "安全降级"],
