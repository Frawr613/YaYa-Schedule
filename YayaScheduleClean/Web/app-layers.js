(function (global) {
  ...
  const originalEffectAnchors = {
    ...
    platform: ["原生 WebView", "通知", "后台/系统提醒管理", "提醒载荷缓存", "通知队列去重", "应用图标", "iOS Widget 刷新合批", "iOS 提醒重复挂载去重", "iOS 通知相同载荷跳过桥接", "备注原生桥接跳过", "iOS 前后台提醒轻量恢复", "iOS 前台恢复合并", "iOS 权限状态查询节流", "iOS 权限状态 UI 去重", "iOS 教务状态回写去重", "iOS 教务回传拉取合并", "iOS 教务桥接能力探测", "iOS 键盘避让桥接", "iOS WebKit 进程恢复", "教务单层网页登录", "教务主题桥接去重"],
  ...
  const layers = [
    ...
    },
    {
      ...
      owns: ["原生保存", "通知", "后台/系统提醒管理", "提醒载荷缓存", "通知队列去重", "应用图标", "iOS Widget 刷新合批", "iOS 提醒重复挂载去重", "iOS 通知相同载荷跳过桥接", "备注原生桥接跳过", "iOS 前后台提醒轻量恢复", "iOS 前台恢复合并", "iOS 权限状态查询节流", "iOS 权限状态 UI 去重", "iOS 教务状态回写去重", "iOS 教务回传拉取合并", "iOS 教务桥接能力探测", "iOS 键盘避让桥接", "iOS WebKit 进程恢复", "教务 WebView", "单层网页登录", "教务主题桥接去重", "Web 预览降级"],
      provides: ["统一平台 API", "原生能力探测", "后台提醒状态", "后台提醒状态节流", "提醒载荷缓存", "通知队列去重", "前台恢复合并触发", "权限状态 UI 去重签名", "通知桥接去重签名", "备注原生桥接跳过", "iOS 键盘高度变量", "iOS WebKit 恢复", "教务导入回传", "教务导入回传合并", "教务状态回写去重", "教务主题桥接去重", "安全降级"],
