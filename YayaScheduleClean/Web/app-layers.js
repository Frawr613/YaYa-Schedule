(function (global) {
  ...
  const originalEffectAnchors = {
    source: ["教务导入", "文件导入", "确认学期", "教务页内确认学期", "结构化 DOM 采集", "网页导出优先", "分页源数据兜底", "网页导出回传", "网页型 XLS", "伪 XLS HTML 解析", "文件导入松散解析"],
  ...
  const layers = [
    {
      ...
      name: "原始数据导入层",
      files: ["app.js"],
      owns: ["教务 HTML", "教务学期字段", "教务分页内容", "教务结构化 DOM", "网页导出文件", "网页型 XLS", "CSV/TXT", "备份 JSON"],
      provides: ["原始课表行", "原始考试行", "结构化 DOM 载荷", "分页采集载荷", "网页导出回传载荷", "学期候选", "备份载荷"],
