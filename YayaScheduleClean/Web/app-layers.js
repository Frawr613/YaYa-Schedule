(function (global) {
  ...
  const originalEffectAnchors = {
    ...
    cache: ["localStorage", "日期索引", "DDL 索引", "日程/特殊变更状态分桶", "DDL 视图状态", "已完成 DDL 检索状态", "HTML 局部缓存", "滚动延迟渲染闸门", "主题变量缓存", "原生桥接去重缓存", "Service Worker"],
    ...
    interaction: ["玻璃浮窗", "本地丝滑弹出动画", "独立自定义主题面", "底部/侧边抽屉", "日期时间选择器", "模板输入浮层", "内置选项弹层", "滚动锁定", "顶层滚动白名单", "滚动不触发日程卡重绘", "渲染运行时节流", "教务页内确认浮窗", "视窗锁定拖动"],
    theme: ["玻璃变量", "暗色玻璃变量", "暗色基底识别", "暗色透明哑光玻璃", "标题卡日程卡同步", "标题左右按钮同步", "标题/小日程默认透明度隔离", "主题保存即时刷新", "暗色反光降噪", "主题预设", "自定义主题栏", "独立自定义主题面", "模板-主题桥接", "输入组件主题变量", "模块语义色", "状态色", "应用图标"],
    template: ["模块顺序", "入口动作", "浮窗策略", "输入组件形态", "主题桥接变量", "模块语义槽位"],
    platform: ["原生 WebView", "通知", "后台/系统提醒管理", "应用图标", "小组件", "Widget 去重刷新", "iOS 提醒重复挂载去重", "教务单层网页登录", "教务主题桥接去重"],
  ...
  const layers = [
    ...
    },
    {
      ...
      name: "本地状态/缓存层",
      files: ["app.js", "sw.js"],
      owns: ["localStorage 状态", "日期索引", "DDL 索引", "日程/特殊变更状态分桶", "DDL 页签状态", "已完成 DDL 检索状态", "HTML 局部缓存", "当天条目缓存", "滚动延迟渲染闸门", "原生桥接去重缓存", "Service Worker 优先缓存"],
      provides: ["当天日程读取", "日程/特殊变更进行中计数", "DDL 快照", "DDL 进行中/已完成快照", "已完成 DDL 检索快照", "局部渲染快照", "滚动后仅补积压渲染", "原生桥接去重签名", "离线资源"],
    ...
    },
    {
      ...
      owns: ["浮窗层级", "独立自定义主题面", "底部抽屉", "侧边抽屉", "居中面板", "选择器浮层", "滚动隔离", "顶层滚动白名单", "滚动不触发日程卡重绘", "渲染运行时节流", "滑动操作", "重渲染保护"],
      provides: ["modalLayout", "density", "浮窗置顶", "下层冻结", "滚动渲染闸门", "运行时刷新预算", "轻触反馈", "繁忙降噪"],
    ...
    },
    {
      ...
      owns: ["主题预设", "自定义主题栏", "独立自定义主题面", "详细变量", "状态色", "玻璃感", "暗色玻璃感", "暗色基底识别", "暗色透明哑光玻璃", "标题卡日程卡同步", "标题左右按钮同步", "标题/小日程默认透明度隔离", "主题保存即时刷新", "暗色反光降噪", "圆角阴影", "输入组件主题变量"],
      provides: ["CSS 变量", "功能状态色", "静态主题预览", "自定义主题即时应用", "自定义主题一次性应用", "模板主题桥接变量", "暗色日程卡槽位", "标题卡日程卡同步槽位", "标题左右按钮同步槽位", "标题/小日程默认透明度槽位", "暗色页面/浮窗反光槽位", "底层暗色透明哑光玻璃变量", "输入组件主题同步变量"],
    ...
    },
    {
      ...
      owns: ["原生保存", "通知", "后台/系统提醒管理", "应用图标", "小组件", "Widget 去重刷新", "iOS 提醒重复挂载去重", "教务 WebView", "单层网页登录", "教务主题桥接去重", "Web 预览降级"],
      provides: ["统一平台 API", "原生能力探测", "后台提醒状态", "Widget 更新", "Widget 去重更新", "教务导入回传", "教务主题桥接去重", "安全降级"],
