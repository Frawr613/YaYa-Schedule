(function (global) {
  ...
  const originalEffectAnchors = {
    ...
    platform: ["原生 WebView", "通知", "后台/系统提醒管理", "提醒载荷缓存", "通知队列去重", "应用图标", "iOS Widget 刷新合批", "iOS 提醒重复挂载去重", "iOS 通知相同载荷跳过桥接", "备注原生桥接跳过", "iOS 前后台提醒轻量恢复", "iOS 前台恢复合并", "iOS 权限状态查询节流", "iOS 权限状态 UI 去重", "iOS 教务状态回写去重", "iOS 教务回传拉取合并", "iOS 教务桥接能力探测", "iOS 键盘避让桥接", "iOS WebKit 进程恢复", "iOS POST 新窗口保真", "iOS 文件导入取消静默", "教务单层网页登录", "教务主题桥接去重"],
  ...
  const layers = [
    ...
    },
    {
      ...
      provides: ["当天日程读取", "持久化去重写入", "缓存结构版本校验", "缓存重建签名复用", "按日期特殊变更读取", "日程/特殊变更进行中计数", "DDL 快照", "DDL 进行中/已完成快照", "DDL 目标计数缓存", "完成项快速判定", "备注计数缓存", "已完成 DDL 检索快照", "已完成 DDL 筛选无变化跳过", "已完成 DDL 筛选结果缓存", "权限提示无变化跳过", "提醒载荷缓存", "提醒载荷归一快照", "提醒载荷计数快照", "通知队列去重", "iOS Widget 主题载荷快照", "教务导入回传合并队列", "局部渲染快照", "浮窗惰性 HTML 快照", "空浮窗无操作快照", "自动定位去重签名", "概览分组去重签名", "概览分组复用缓存签名", "备注变更轻量渲染", "备注原生桥接跳过", "视图状态无变化跳过", "日期聚焦无变化跳过", "滚动标记低频刷新", "滚动后合并强制渲染", "渲染 RAF 合并", "架构运行时去重签名", "模板装配去重签名", "主题变量去重签名", "缓存重建去重签名", "前台恢复合并触发", "权限状态 UI 去重签名", "通知桥接去重签名", "滚动后仅补积压渲染", "主题/模板去重签名", "原生桥接去重签名", "离线资源"],
    ...
    },
    {
      ...
      owns: ["原生保存", "通知", "后台/系统提醒管理", "提醒载荷缓存", "通知队列去重", "应用图标", "iOS Widget 刷新合批", "iOS 提醒重复挂载去重", "iOS 通知相同载荷跳过桥接", "备注原生桥接跳过", "iOS 前后台提醒轻量恢复", "iOS 前台恢复合并", "iOS 权限状态查询节流", "iOS 权限状态 UI 去重", "iOS 教务状态回写去重", "iOS 教务回传拉取合并", "iOS 教务桥接能力探测", "iOS 键盘避让桥接", "iOS WebKit 进程恢复", "iOS POST 新窗口保真", "iOS 文件导入取消静默", "教务 WebView", "单层网页登录", "教务主题桥接去重", "Web 预览降级"],
      provides: ["统一平台 API", "原生能力探测", "后台提醒状态", "后台提醒状态节流", "提醒载荷缓存", "通知队列去重", "前台恢复合并触发", "权限状态 UI 去重签名", "通知桥接去重签名", "备注原生桥接跳过", "iOS 键盘高度变量", "iOS WebKit 恢复", "iOS 原始请求承接", "iOS 文件导入取消事件", "教务导入回传", "教务导入回传合并", "教务状态回写去重", "教务主题桥接去重", "安全降级"],
