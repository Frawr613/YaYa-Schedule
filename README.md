...
这是鸦鸦日程 2.0 的 iOS 适配工程包，不内置个人课表数据。工程以当前 Web 应用为核心界面，外层使用 iOS 原生 Swift + WKWebView 承载，并接入 iOS 通知、小组件、教务导入和前端平台桥。

## 已包含

- 主 App：`YayaScheduleClean`
- iOS 小组件扩展：`YayaScheduleWidget`
- 当前 2.0 Web 资源：十层结构、模板/主题桥接、内置输入组件、DDL 分页、特殊变更等
- iOS 平台桥：`YayaNative`
  - `savePortalAccount`
  - `openAcademicPortal`
  - `takeImportedPage`
  ...
  - `scheduleDdlNotifications`
  - `updateHomeWidget`
- 教务导入页浮层按钮：导入课表、导入考试、返回鸦鸦
- 通知：DDL/日程提醒调度，最多保留最近 64 条待触发提醒
- 小组件：最近 DDL + 最近课程/日程，进行中日程显示进度
